...
export default class extends BaseSeeder {
  ...
  async #createMovies(cineasts: Cineast[]): Promise<void> {
    let index = 0
    ...
    const promises = movieRecords.map(async (movie) => {
      ...
      return this.#attachRandomCrewMembers(movie, cineasts, 5)
    })
  ...
  async #attachRandomCrewMembers(movie: Movie, cineasts: Cineast[], number: number) {
    const cineastIds = this.#getRandom(cineasts, number).map(({ id }) => id)

    return movie.related('crewMembers').attach(
      cineastIds.reduce<Record<string, ModelObject>>((obj, id, i) => {
        obj[id] = {
